...
class BiPCA(BaseEstimator):
    ...
    if logger == None:
        ...
        self.logger = tasklogger.TaskLogger(name='Shrinker ' 
        	+ str(Shrinker.__log_instance),level = verbose)
    else:
        self.logger = logger
    #initialize the subprocedure classes
    self.k = n_components
    self._sinkhorn = Sinkhorn(tol = tol, n_iter = n_iter, verbose = verbose)
    self._svd = SVD(n_components = n_components, exact=exact, logger=logger)
	self._shrinker = Shrinker(default_shrinker=default_shrinker, 
								rescale_svs = True, verbose=verbose, logger = logger)
	@property
	...
	@property
	def Z(self):
		return self._sinkhorn.Z
	def _unscale(self,X):
		return self._sinkhorn.unscale(X)
	def fit(self, X):
		self.k = np.min([self.k, *X.shape]) #ensure we are not asking for too many SVs
		self._sinkhorn.fit(X)
		self._svd.k = self.k
		self._svd.fit(self._sinkhorn.Z)
		self._shrinker.fit(self.S, shape = M.shape)
		self._mp_rank = self._shrinker.scaled_mp_rank
		self.fit_ = True
